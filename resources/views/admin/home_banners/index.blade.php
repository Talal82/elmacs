		...
		<div class="card">
			<div class="card-header">
				...
				<div class="pull-right">
					<a class="btn btn-primary" href="{{ route('admin.home-banners.create') }}"><i class="fa fa-plus"></i> New</a>
				</div>
			</div>
			<div class="card-body">
				@if(count($banners) > 0)
				<table class="table">
					...
					<tbody>
						...
						<tr>
							<td><input type="checkbox" class="checkbox checkbox-custom" data-id="{{$banner->id}}"></td>
							<td>{{ ++$key }}</td>
							<td>{{ $banner -> text }}</td>
							<td>
								<img src="{{ asset('public/uploads/images/banners/'. $banner -> image) }}" width="200" height="80">
							</td>
							
							<td>
								<a href="{{ route('admin.home-banners.edit', [ $banner -> id ]) }}" class="btn btn-primary btn-sm pull-left m-r-5 waves waves-effect" title="Edit"><i class="fa fa-wrench" title="Edit"></i></a>

								<a href="javascript:void(0);" data-id="{{ $banner-> id }}" class="sa-remove waves wave-effect btn btn-danger btn-sm pull-left" title="Delete"><i class="fa fa-trash"></i></a>
								
								{!! Form::open(['route' => ['admin.home-banners.destroy', $banner -> id], 'method' => 'DELETE', 'id' => $banner -> id]) !!}
...
<script>
  var url = 'delete-multiple-home-banners';
</script>
