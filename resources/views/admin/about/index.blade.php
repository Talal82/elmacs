		...
		<div class="card">
			<div class="card-header">
				...
				<div class="pull-right">
					<a class="btn btn-primary" href="{{ route('admin.about.create') }}"><i class="fa fa-plus"></i> New</a>
				</div>
			</div>
			<div class="card-body">
				@if(count($abouts) > 0)
				<table class="table">
					...
					<tbody>
						...
						<tr>
							<td><input type="checkbox" class="checkbox checkbox-custom" data-id="{{$about->id}}"></td>
							<td>{{ ++$key }}</td>
							<td>{{ $about -> tab_name }}</td>
							<td>{!! (strlen($about -> tab_detail) > 300) ? substr($about -> tab_detail, 0, 300).'...' : $about -> tab_detail !!}</td>
							
							<td>
								<a href="{{ route('admin.about.edit', [ $about -> id ]) }}" class="btn btn-primary btn-sm pull-left m-r-5 waves waves-effect" title="Edit"><i class="fa fa-wrench" title="Edit"></i></a>

								<a href="javascript:void(0);" data-id="{{ $about-> id }}" class="sa-remove waves wave-effect btn btn-danger btn-sm pull-left" title="Delete"><i class="fa fa-trash"></i></a>
								

								{!! Form::open(['route' => ['admin.about.destroy', $about -> id], 'method' => 'DELETE', 'id' => $about -> id]) !!}
