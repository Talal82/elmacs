		...
		<div class="card">
			<div class="card-header">
				...
				<div class="pull-right">
					<a class="btn btn-primary" href="{{ route('admin.advertisements.create') }}"><i class="fa fa-plus"></i> New</a>
				</div>
			</div>
			<div class="card-body">
				...
				<table class="table">
					...
					<tbody>
						...
						<tr>
							<td><input type="checkbox" class="checkbox checkbox-custom" data-id="{{$advertisement->id}}"></td>
							<td>{{ ++$key }}</td>
							<td>{{ $advertisement -> name }}</td>
							<td>{{ $advertisement -> count }}</td>
							
							<td>
								<a href="{{ route('admin.advertisements.edit', [ $advertisement -> id ]) }}" class="btn btn-primary btn-sm pull-left m-r-5 waves waves-effect" title="Edit"><i class="fa fa-wrench" title="Edit"></i></a>

								<a href="javascript:void(0);" data-id="{{ $advertisement-> id }}" class="sa-remove waves wave-effect btn btn-danger btn-sm pull-left" title="Delete"><i class="fa fa-trash"></i></a>
								

								{!! Form::open(['route' => ['admin.advertisements.destroy', $advertisement -> id], 'method' => 'DELETE', 'id' => $advertisement -> id]) !!}
...
<script>
  var url = 'delete-multiple-advertisements';
</script>
