		...
		<div class="card">
			<div class="card-header">
				...
				<div class="pull-right">
					<a class="btn btn-primary" href="{{ route('admin.news.create') }}"><i class="fa fa-plus"></i> New</a>
				</div>
			</div>
			<div class="card-body">
				@if(count($news) > 0)
				<table class="table">
					...
					<tbody>
						...
						<tr>
							<td><input type="checkbox" class="checkbox checkbox-custom" data-id="{{$event->id}}"></td>
							<td>{{ ++$key }}</td>
							<td>{{ $event -> heading }}</td>
							<td>
								<img src="{{ asset('public/uploads/images/news/'. $event -> image) }}" width="100" height="100">
							</td>
							<td>{!! (strlen($event -> detail) > 300) ? substr($event -> detail, 0, 300).'...' : $event -> detail !!}</td>
							
							<td>
								<a href="{{ route('admin.news.edit', [ $event -> id ]) }}" class="btn btn-primary btn-sm pull-left m-r-5 waves waves-effect" title="Edit"><i class="fa fa-wrench" title="Edit"></i></a>

								<a href="javascript:void(0);" data-id="{{ $event-> id }}" class="sa-remove waves wave-effect btn btn-danger btn-sm pull-left" title="Delete"><i class="fa fa-trash"></i></a>
								

								{!! Form::open(['route' => ['admin.news.destroy', $event -> id], 'method' => 'DELETE', 'id' => $event -> id]) !!}
