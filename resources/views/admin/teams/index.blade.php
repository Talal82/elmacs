		...
		<div class="card">
			<div class="card-header">
				...
				<div class="pull-right">
					<a class="btn btn-primary" href="{{ route('admin.teams.create') }}"><i class="fa fa-plus"></i> New</a>
				</div>
			</div>
			<div class="card-body">
				@if(count($teams) > 0)
				<table class="table">
					...
					<tbody>
						...
						<tr>
							<td><input type="checkbox" class="checkbox checkbox-custom" data-id="{{$member->id}}"></td>
							<td>{{ ++$key }}</td>
							<td>{{ $member -> name }}</td>
							<td>{{ $member -> role }}</td>
							<td>
								<img src="{{ asset('public/uploads/images/teams/'. $member -> image) }}" width="100" height="100">
							</td>
							<td>{{ (strlen($member -> detail) > 170) ? substr($member -> detail, 0, 170).'...' : $member -> detail }}</td>
							
							<td>
								<a href="{{ route('admin.teams.edit', [ $member -> id ]) }}" class="btn btn-primary btn-sm pull-left m-r-5 waves waves-effect" title="Edit"><i class="fa fa-wrench" title="Edit"></i></a>

								<a href="javascript:void(0);" data-id="{{ $member-> id }}" class="sa-remove waves wave-effect btn btn-danger btn-sm pull-left" title="Delete"><i class="fa fa-trash"></i></a>
								

								{!! Form::open(['route' => ['admin.teams.destroy', $member -> id], 'method' => 'DELETE', 'id' => $member -> id]) !!}
