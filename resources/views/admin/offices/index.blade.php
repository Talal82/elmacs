		...
		<div class="card">
			<div class="card-header">
				...
				<div class="pull-right">
					<a class="btn btn-primary" href="{{ route('admin.offices.create') }}"><i class="fa fa-plus"></i> New</a>
				</div>
			</div>
			<div class="card-body">
				@if(count($offices) > 0)
				<table class="table table-responsive">
					...
					<tbody>
						...
						<tr>
							<td><input type="checkbox" class="checkbox checkbox-custom" data-id="{{$office->id}}"></td>
							<td>{{ ++$key }}</td>
							<td>{{ $office -> name }}</td>
							<td>{{ strlen($office -> map_link) > 80 ? substr($office -> map_link, 0, 80).'...' : $office -> map_link }}</td>
							<td>{!! strlen($office -> detail) > 150 ? substr($office -> detail, 0, 150).'...' : $office -> detail !!}</td>
							
							<td>
								<a href="{{ route('admin.offices.edit', [ $office -> id ]) }}" class="btn btn-primary btn-sm pull-left m-r-5 waves waves-effect" title="Edit"><i class="fa fa-wrench" title="Edit"></i></a>

								<a href="javascript:void(0);" data-id="{{ $office-> id }}" class="sa-remove waves wave-effect btn btn-danger btn-sm pull-left" title="Delete"><i class="fa fa-trash"></i></a>
								
								{!! Form::open(['route' => ['admin.offices.destroy', $office -> id], 'method' => 'DELETE', 'id' => $office -> id]) !!}
...
<script>
  var url = 'delete-multiple-offices';
</script>
